...
const welcome = async member => {
  await loaded.channels.lobby.send(
    `
${member} welcome to PwnSquad! Make sure to read the ${
      loaded.channels.rules
    } and get some cool ${loaded.channels.roles}.
...
client.on("ready", async () => {
  ...
  const task = async () => {
    ...
    freshWanderingRole.members.forEach(async member => {
      ...
      if (daysLeft > 1) {
        message += ` you have **${daysLeft} days** left to send your verification message! See ${
          loaded.channels.info
        } to learn how`;
      } else if (daysLeft === 1) {
        try {
          await member.send(
            `Just a friendly reminder, you only have **1 day** left to become verified on PwnSquad or you'll be **kicked**! *Don't worry, all you have to do is tell us a little about yourself :)*`
          );
        } catch (error) {
          console.log(`> Couldn't send a DM to ${member.displayName}`);
        }
        message += ` you have **1 day** left to send your verification message, and you're really close to being **kicked**. See ${
          loaded.channels.info
        } to learn more`;
      } else {
        await member.kick();
        try {
          await member.send(
            `Sorry, but you've kicked from PwnSquad for not sending your verification message fast enough. In our defense, we warned you ahread of time! *If you want to try again feel free to join back*`
          );
        } catch (error) {
          console.log(`> Couldn't send a DM to ${member.displayName}`);
        }
...
client.on("message", async message => {
  if (
    ...
    (message.content.includes("81ZH2Y") ||
      message.content.includes("nakedphotos.club") ||
      message.content.includes("privatepage.vip") ||
      message.content.includes("18+ photos"))
  ) {
  ...
  if (message.content.startsWith(`${prefix}verify`)) {
    if (members.size < 1) {
      await message.channel.send(
        `${loaded.emojis.no} You must specify a member to verify!`
      );
      return;
    }
    ...
    if (members.size < 1) {
      await message.channel.send(
        `${loaded.emojis.no} You must specify a member to reject!`
      );
      return;
    }
    ...
    for (let [_, member] of members) {
      if (member.roles.get(loaded.roles.verified.id)) {
        await message.channel.send(
          `${
            loaded.emojis.no
          } ${member} is verified, so you can't reject them! You may want to kick them instead.`
        );
      } else {
        try {
          await member.send(
            "Sorry, but your application has been rejected. If you think you can do better fill out the form again. "
          );
        } catch (error) {
          console.log(`> Couldn't send a DM to ${member.displayName}`);
        }
        await member.kick();
        await message.channel.send(
          `${loaded.emojis.yes} ${member} has been rejected.`
        );
      }
    }
  }
