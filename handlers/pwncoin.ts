...
const apiUrl =
	'https://v2-api.sheety.co/fa7b3330a4fd8309592da41028e8dce4/pwnCoin/balances'
	...
	async onMessage(message: Message) {
		if (message.content.startsWith(`${prefix}bal`)) {
			...
			await message.channel.send(
				`\`${user.tag}\` has **${amount}** ${this.loaded.emojis.pwncoin}`
			)
			return true
		}
		...
		if (message.content.startsWith(`${prefix}lead`)) {
			...
			const firstTenBalances = await Promise.all(
				balances.slice(0, 10).map(async ({ id, amount }) => ({
			...
			const leaders = firstTenBalances
				.map(
					({ user, amount }, index) =>
						`${index + 1}. \`${user.tag}\` has **${amount}** PwnCoin`
				)
				.join('\n')

			await message.channel.send(
				`${this.loaded.emojis.pwncoin} **Leaderboard:**\n${leaders}`
			)
			return true
		}
		...
		if (message.content.startsWith(`${prefix}tot`)) {
			...
			await message.channel.send(
				`There's **${total}** ${this.loaded.emojis.pwncoin} in circulation!`
			)
			return true
		}
